...
Engine::Engine() :
 hud( Hud::getInstance() ),
  ...
  renderer( rc->getRenderer() ),
   blank("blank", Gamedata::getInstance().getXmlInt("blank/factor") ),
  cloud("cloud", Gamedata::getInstance().getXmlInt("cloud/factor") ),
  mountain("mountain", Gamedata::getInstance().getXmlInt("mountain/factor") ),
  plant("plant", Gamedata::getInstance().getXmlInt("plant/factor") ),
  base("base", Gamedata::getInstance().getXmlInt("base/factor") ),
  viewport( Viewport::getInstance() ),
   player(new ShootingSprite("Bird-right","Bird-left")),
  sprites(),
  sprit(),
  snow(),
  strategies(),
...
sound()
{
 int n = Gamedata::getInstance().getXmlInt("numberOfSmartSprites");
  sprites.reserve(n);
 Vector2f pos = player->getPosition();
  int w = player->getScaledWidth();
  int h = player->getScaledHeight();
  for (int i = 0; i < n; ++i) {
    sprites.push_back( new SmartSprite("Enemyright","Enemyleft", pos, w, h) );
  ...
  sprit.push_back(new MultiSprite("laser1"));
  sprit.push_back(new MultiSprite("laser2"));
  sprit.push_back(new MultiSprite("laser3"));
...
SDL_Color color2 = {255,0,128,255};
...
unsigned int sec = Gamedata::getInstance().getXmlInt("hud/seconds");
  ...
  if(displayhud%2!=0 || clock.getSeconds()<sec){
   ...
   IOmod::getInstance().writeText("Namitha Tumkur",color1,30,450);


 	
std::stringstream lives;
    lives << "lives remaining: " << life;
